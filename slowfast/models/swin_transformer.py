...
class VideoSwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dim = cfg.VST.EMBED_DIM
        self.patch_norm = cfg.VST.PATCH_NORM
        self.frozen_stages = cfg.VST.FROZEN_STAGES
        self.window_size = cfg.VST.WINDOW_SIZE
        self.patch_size = cfg.VST.PATCH_SIZE
        num_heads = cfg.VST.NUM_HEADS
        depths = cfg.VST.DEPTHS
        drop_rate = cfg.VST.DROP_RATE
        use_checkpoint= cfg.VST.USE_CHECKPOINT
        
        # split image into non-overlapping patches
        self.patch_embed = PatchEmbed3D(
            patch_size=self.patch_size, in_chans=cfg.VST.IN_CHANS, embed_dim=self.embed_dim,
            norm_layer=norm_layer if self.patch_norm else None)

        self.pos_drop = nn.Dropout(p=drop_rate)
        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, cfg.VST.DROP_PATH_RATE, sum(depths))]  # stochastic depth decay rule
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                mlp_ratio=cfg.VST.MLP_RATIO,
                qkv_bias=cfg.VST.QKV_BIAS,
                qk_scale=cfg.VST.QK_SCALE,
                drop=drop_rate,
                attn_drop=cfg.VST.ATTN_DROP_RATE,
    ...
    def forward(self, x, bboxes=None, features=None):
        ...
        thw = [x.shape[1], x.shape[2], x.shape[3]]
        x = self.norm(x)
    ...
    def train(self, mode=True):
        ...
        super(VideoSwinTransformer, self).train(mode)
        self._freeze_stages()
