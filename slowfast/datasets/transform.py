...
def gaussian_blur(prob, images, boxes=None, kernel_size=3):
    """
    Perform gaussian blur on the given images.
    Args:
        prob (float): probability to apply gaussian blurr on the images.
        images (tensor): images to perform horizontal flip, the dimension is
            `num frames` x `channel` x `height` x `width`.
        boxes (ndarray or None): optional. Corresponding boxes to images.
            Dimension is `num boxes` x 4.
    Returns:
        images (tensor): images with dimension of
            `num frames` x `channel` x `height` x `width`.
        flipped_boxes (ndarray or None): the boxes with dimension of
            `num boxes` x 4.
    """
    if boxes is None:
        flipped_boxes = None
    ...
    if np.random.uniform() < prob:
        images = transforms.GaussianBlur(kernel_size, sigma=(0.1, 2.0))(images)

    return images, flipped_boxes

def rand_grayscale(prob, images, boxes=None):
    """
    Perform gaussian blur on the given images.
    Args:
        prob (float): probability to apply gaussian blurr on the images.
...
def rand_color_jitter(prob, images, img_brightness, img_contrast, img_saturation, boxes=None):
    """
    Perform gaussian blur on the given images.
    Args:
        prob (float): probability to apply gaussian blurr on the images.
    ...
    if np.random.uniform() < prob:
        images = color_jitter(images, img_brightness, img_contrast, img_saturation)
